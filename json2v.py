...
class Color:
    ENABLED = True
    @staticmethod
    def _w(c, t): return t if not Color.ENABLED or not sys.stderr.isatty() else f"\033[{c}m{t}\033[0m"
    @staticmethod
    def red(t): return Color._w("31", t)
    @staticmethod
    def green(t): return Color._w("32", t)
    @staticmethod
    def yellow(t): return Color._w("33", t)
    @staticmethod
    def blue(t): return Color._w("34", t)
    @staticmethod
    def cyan(t): return Color._w("36", t)
    @staticmethod
    def bold(t): return Color._w("1", t)
    @staticmethod
    def dim(t): return Color._w("2", t)
...
def infer_list(items, key, ctx):
    ...
    if len(items) > MAX_SAMPLE:
        ctx.issues.append(Issue(Level.WARN, "V005", f"List '{ctx.path}' truncated to {MAX_SAMPLE} for inference", ctx.path, ""))
    types = []
    field_types = {}
    for it in sample:
        t = type(it).__name__
        if isinstance(it, dict):
            for k, v in it.items(): field_types.setdefault(k, set()).add(type(v).__name__)
        types.append(t)
...
def lint_name(key, ctx):
    if key in KWS:
        new = f"{key}_"
        ctx.issues.append(Issue(Level.WARN if ctx.fix else Level.ERROR, "V010", f"'{ctx.path}' is reserved", ctx.path, f"-> {new}" if ctx.fix else f"Rename to {new}"))
        return new if ctx.fix else key
    if key in BUILTINS:
        new = f"{key}_val"
        ctx.issues.append(Issue(Level.WARN if ctx.fix else Level.ERROR, "V011", f"'{ctx.path}' conflicts builtin", ctx.path, f"-> {new}" if ctx.fix else f"Rename to {new}"))
        return new if ctx.fix else key
    if not is_snake(key):
        snake = c2s(key)
        if not key.isascii() or snake != key:
            ctx.issues.append(Issue(Level.WARN, "V012", f"'{ctx.path}' invalid naming", ctx.path, f"-> {snake}" if ctx.fix else "Use ASCII snake_case"))
            return snake if ctx.fix and snake != key else key
    if key and key[0].isdigit():
        new = f"f_{key}"
        ctx.issues.append(Issue(Level.WARN if ctx.fix else Level.ERROR, "V013", f"'{ctx.path}' starts digit", ctx.path, f"-> {new}" if ctx.fix else f"Rename to {new}"))
        return new if ctx.fix else key
    return key

def convert(key, val, ctx):
    if ctx.depth > MAX_DEPTH:
        ctx.issues.append(Issue(Level.ERROR, "V099", f"Max depth at '{ctx.path}'", ctx.path, "Flatten JSON"))
    ...
    if val is None:
        ctx.issues.append(Issue(Level.WARN, "V003", f"Null at '{path}'", path, "Replace ?any"))
        parts.append(f"{pfx}mut {resolved} ?any"); return "\n".join(parts)
    t = type(val).__name__
    if t == "bool": parts.append(f"{pfx}mut {resolved} bool")
    elif t == "int": parts.append(f"{pfx}mut {resolved} int")
    elif t == "float": parts.append(f"{pfx}mut {resolved} f64")
    elif t == "str": parts.append(f"{pfx}mut {resolved} string")
    elif t == "list":
        ...
        if val and isinstance(val[0], dict):
            ...
            for it in val:
                if isinstance(it, dict):
                    for k, v in it.items(): merged.setdefault(k, v)
        ...
        sub = ctx.sub(path)
        for k, v in val.items(): parts.append(convert(k, v, sub.sub(f"{path}.{k}")))
        parts.append(f"{pfx}}}")
    else:
        ctx.issues.append(Issue(Level.ERROR, "V004", f"Unmappable {t} at '{path}'", path, "Specify V type"))
        parts.append(f"{pfx}mut {resolved} any")
    return "\n".join(parts)

def gen(data, root, fix):
    issues, nested = [], []
    ctx = Ctx(issues, fix, nested, 4, root, 0)
    parts = ["import json\n", f"pub struct {root} {{"]
    for k, v in data.items(): parts.append(convert(k, v, ctx.sub(f"{root}.{k}")))
    parts.append("}")
    for ns in nested: parts.append(f"\n{ns}\n")
    if fix:
        seen = {}
        for k in data:
            tc = Ctx([], True, [], 0, f"{root}.{k}", 0)
            fixed = lint_name(k, tc)
            if fixed in seen: issues.append(Issue(Level.ERROR, "V014", f"Collision: '{seen[fixed]}' & '{k}' -> '{fixed}'", f"{root}.{fixed}", "Resolve manually"))
            seen[fixed] = k
    return "\n".join(parts), issues

def rpt(issues, verbose):
...
def main():
    ...
    if not isinstance(data, dict):
        ...
        print(rpt(issues, args.verbose), file=sys.stderr); sys.exit(1)
    ...
    if args.strict:
        ...
        if ec or wc:
            ...
            print(Color.red(f"✗ Strict: {ec} err, {wc} warn"), file=sys.stderr); sys.exit(1)
    if args.dry_run:
